...
public class DepositProcessorTests {
    ...
    protected void deposit_processors_can_handle_deposit_transactions_to_checking() {
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, id, depositAmount)));
    ...
    protected void deposit_processors_can_ignore_additional_arguments() {
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s", transactionType, CHECKING_ID, bank.getAccount(CHECKING_ID).getMaxDepositAmount(), "nuke")));
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s %s %s  %s %s  %s %s", transactionType, SAVINGS_ID, bank.getAccount(SAVINGS_ID).getMaxDepositAmount(), "0", "0", "0", "0", "0", "0", "0")));
    }

    @Test
    protected void deposit_processors_are_case_insensitive() {
        assertTrue(processor.handleTransaction(String.format("dePoSIT %s %s", CHECKING_ID, bank.getAccount(CHECKING_ID).getMaxDepositAmount())));
        assertTrue(processor.handleTransaction(String.format("deposit %s %s", SAVINGS_ID, bank.getAccount(SAVINGS_ID).getMaxDepositAmount())));
    }
    ...
    protected void deposit_processors_can_be_in_a_chain_of_responsibility() {
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, payingID, depositAmount)));

        processor.setNext(new WithdrawProcessor(bank));

        assertTrue(processor.handleTransaction(String.format("%s %s %s", TransactionType.Withdraw, payingID, withdrawAmount)));
        assertEquals(depositAmount - withdrawAmount, bank.getAccount(payingID).getBalance());
        assertFalse(processor.handleTransaction(String.format("%s %s %s %s", TransactionType.Transfer, payingID, receivingID, transferAmount)));
    }
}
