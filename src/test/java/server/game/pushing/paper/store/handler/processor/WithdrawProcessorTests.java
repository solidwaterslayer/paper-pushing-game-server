...
public class WithdrawProcessorTests {
    ...
    protected void withdraw_processors_can_withdraw_from_checking_accounts_when_the_withdraw_amount_is_less_than_the_account_balance() {
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, id, withdrawAmount)));
    ...
    protected void withdraw_processors_can_withdraw_when_the_withdraw_amount_is_equal_to_the_account_balance() {
        double checkingWithdrawAmount = timeTravel(checkingDepositAmount, MONTHS);
        double savingsWithdrawAmount = timeTravel(savingsDepositAmount, MONTHS);
        double cdWithdrawAmount = timeTravel(cdBalance, MONTHS);
        bank.timeTravel(MONTHS);

        assertEquals(checkingWithdrawAmount, bank.getAccount(CHECKING_ID).getBalance());
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, CHECKING_ID, checkingWithdrawAmount)));
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, SAVINGS_ID, savingsWithdrawAmount)));
        assertEquals(0, bank.getAccount(SAVINGS_ID).getBalance());

        assertEquals(cdWithdrawAmount, bank.getAccount(CD_ID).getBalance());
        assertTrue(processor.handleTransaction(String.format("%s %s %s", transactionType, CD_ID, cdWithdrawAmount)));
    ...
    protected void withdraw_processors_can_ignore_additional_arguments() {
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s", transactionType, CHECKING_ID, bank.getAccount(CHECKING_ID).getMaxWithdrawAmount(), "nuke")));
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s %s  %s    %s         %s", transactionType, SAVINGS_ID, bank.getAccount(SAVINGS_ID).getMaxWithdrawAmount(), "00", "000", "00000", "000", 0)));
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s %s %s", transactionType, CD_ID, bank.getAccount(CD_ID).getMaxWithdrawAmount(), "d", "e", "r")));
    }
    ...
    protected void withdraw_processors_are_case_insensitive() {
        ...
        assertTrue(processor.handleTransaction(String.format("withdraw %s %s", CHECKING_ID, bank.getAccount(CHECKING_ID).getMaxWithdrawAmount())));
        assertTrue(processor.handleTransaction(String.format("wITHdrAw %s %s", SAVINGS_ID, bank.getAccount(SAVINGS_ID).getMaxWithdrawAmount())));
        assertTrue(processor.handleTransaction(String.format("WITHDRAW %s %s", CD_ID, bank.getAccount(CD_ID).getMaxWithdrawAmount())));
    }
    ...
    protected void withdraw_processors_can_be_in_a_chain_of_responsibility() {
        ...
        assertTrue(processor.handleTransaction(String.format("%s %s %s %s", TransactionType.Transfer, payingID, receivingID, transferAmount)));
        ...
        assertFalse(processor.handleTransaction(String.format("%s %s", TransactionType.TimeTravel, MONTHS)));
    }
}
